...
export const Rocket = () => {
    ...
    useFrame((_, delta) => {
        ...
        const velocityMeters = Math.hypot(...velocityRef.current) * METERS_PER_UNIT;
    ...
    return (_jsxs("group", { ref: ref, dispose: null, children: [_jsx("primitive", { object: rocketScene, scale: 0.015, position: [0, -6, 0] }), _jsx(EnginePlume, {})] }));
};
const EnginePlume = () => {
    const throttle = useSimulationStore((state) => state.throttle);
    return (_jsxs("mesh", { position: [0, -10.5, 0], rotation: [Math.PI, 0, 0], children: [_jsx("coneGeometry", { args: [0.8, 4, 32, 1, true] }), _jsx("meshBasicMaterial", { color: "#ffb347", transparent: true, opacity: 0.35 + throttle * 0.45 })] }));
};
